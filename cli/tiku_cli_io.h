 ...
 *
 * tiku_cli_io.h - I/O abstraction for the CLI
 *
...
#define TIKU_CLI_IO_CRLF   0x01

/** Echo received characters back to the sender */
#define TIKU_CLI_IO_ECHO   0x02
 ...
 * tiku_cli_io_set_backend().  The CLI never touches hardware
...
typedef struct {
    ...
    uint8_t flags;                  /**< Bitwise OR of TIKU_CLI_IO_* */
} tiku_cli_io_t;
 ...
 */
void tiku_cli_io_set_backend(const tiku_cli_io_t *backend);

/**
 * @brief Return the currently active backend (or NULL).
 */
const tiku_cli_io_t *tiku_cli_io_get_backend(void);
 ...
 *
 * No CRLF conversion — use tiku_cli_io_puts/printf for that.
 */
 ...
 *
 * @return Non-zero if tiku_cli_io_getc() will return a byte.
 */
 ...
 */
#define CLI_PRINTF(...) tiku_cli_io_printf(__VA_ARGS__)

/*---------------------------------------------------------------------------*/
/* PRE-DEFINED BACKENDS                                                      */
/*---------------------------------------------------------------------------*/

/** UART backend (serial terminal, echo + CRLF). Defined in tiku_cli_io.c. */
extern const tiku_cli_io_t tiku_cli_io_uart;
